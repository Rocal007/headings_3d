...
* **Dateien**: [`src/components/CraneTennis.tsx`](file:///e:/3D-headings/src/components/CraneTennis.tsx), [`src/model/Supertechno50FBXModel.ts`](file:///e:/3D-headings/src/model/Supertechno50FBXModel.ts), [`src/components/CraneTennisRacketHead.tsx`](file:///e:/3D-headings/src/components/CraneTennisRacketHead.tsx)
* **Zuständigkeit**:
  * **Dual-Kran Match Kinematik**: 2 synchrone Supertechno 50 Kräne auf parallelen Grundlinien-Schienen ($Z = -16.8\,\text{m}$ und $+16.8\,\text{m}$, $4.91\,\text{m}$ hinter der Grundlinie) mit dynamischer Inverse Kinematik (IK) für Vorhand, Rückhand, Slice, Topspin, 234 km/h Power-Aufschläge sowie **248 km/h Monster-Smashes & Netz-Volleys** direkt am Netz ($Z \approx \pm 1.8\,\text{m} \dots \pm 3.8\,\text{m}$, Auslegervorschub bis zu 11.4m).
  * **Dedizierter Top-Mounted Tennis-Racket-Head (`CraneTennisRacketHead`)**: Stehend nach oben montierter 3-Achsen Servo-Aktuator (Mitchell-Mount Overslung), der dem Ausleger niemals im Weg ist, mit $1.9\times$ Carbon-Racket, Sweet-Spot Tracker, dynamischer Saitenbeleuchtung und Racket-POV-Kamera.
