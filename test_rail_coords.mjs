...
console.log('--- TEST RAIL & HEAD COORDINATES ---');
for (let ext = 0; ext <= 11.4; ext += 3.8) {
  const t = ext / 11.4;
  ...
  const zStart = 0.5;
  const zEnd = headLocal.z;
  const span = Math.abs(zEnd - zStart);
  console.log(`Ext: ${ext.toFixed(1)}m | Head Local: (${headLocal.x.toFixed(2)}, ${headLocal.y.toFixed(2)}, ${headLocal.z.toFixed(2)}) | Rail Span: ${span.toFixed(2)}m`);
}
